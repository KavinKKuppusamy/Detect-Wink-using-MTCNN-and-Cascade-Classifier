...
def detect_eyes(cascade, frame , face_box) :
    gray_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY) 
    scaleFactor = 1.3 # range is from 1 to ..
    minNeighbors = 7   # range is from 0 to ..
    ...
    eyes = cascade.detectMultiScale(
        ...
        flag, 
        minSize)
    #print("First classifier ", eyes)
    if len(eyes) == 0:
        scaleFactor = 1.04 # range is from 1 to ..
...
def wink(frame, f) :
    ...
    eye_detections = detect_eyes(eye_cascade, frame , box)
    return len(eye_detections) == 1
